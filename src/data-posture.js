...
// rounded progress bar plugin block
const roundedProgressBar = {
  ...
  beforeDatasetsDraw(chart, args, pluginOptions) {
    ...
    chart.getDatasetMeta(0).data.forEach((datapoint, index) => {
      ...
      ctx.fillStyle = data.datasets[0].borderColor[index];
      ctx.lineWidth = barHeight * 0.7; // 0.8 ?
      ctx.lineJoin = "round";
      ctx.strokeRect(left + 4, datapoint.y, width - 5, 1);
      ...
      ctx.fillText(
        "Mauvaise",
        left,
        y.getPixelForValue(index) - fontSizeLabel - 10
      );
      // Label Moyenne middle
      ...
      // ctx.textAlign = 'left';
      // ctx.textBaseline = 'middle';
      // ctx.fillText('Bonne', right, y.getPixelForValue(index) - fontSizeLabel - 10);
      ...
      ctx.fillText(
        "Bonne",
        right,
        y.getPixelForValue(index) - fontSizeDatapoint - 10
      );
    });
  },
};
